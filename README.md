...
本项目实现多种机器学习和深度学习模型，基于借款人信息预测贷款状态。
...
- `person_age`: Age of the person / 借款人年龄
- `person_income`: Annual income / 年收入
- `person_home_ownership`: Home ownership status (RENT, OWN, MORTGAGE, OTHER) / 房屋所有权状态（租房、自有、抵押、其他）
- `person_emp_length`: Employment length in years / 工作年限（年）
- `loan_intent`: Purpose of the loan / 贷款用途
- `loan_grade`: Loan grade (A-G) / 贷款等级（A-G）
- `loan_amnt`: Loan amount / 贷款金额
- `loan_int_rate`: Interest rate / 利率
- `loan_percent_income`: Loan amount as percentage of income / 贷款占收入百分比
- `cb_person_default_on_file`: Historical default (Y/N) / 历史违约记录（是/否）
- `cb_person_cred_hist_length`: Credit history length / 信用历史长度
- `loan_status`: Target variable (0 = No Default, 1 = Default) / 目标变量（0 = 未违约，1 = 违约）
...
### Full Training with Options / 带参数的完整训练
...
#### Options / 参数说明:
- `--train`: Path to training CSV (required) / 训练数据 CSV 路径（必需）
- `--test`: Path to test CSV (required) / 测试数据 CSV 路径（必需）
- `--original`: Path to original loan data CSV (optional) / 原始贷款数据 CSV 路径（可选）
...
- `--ensemble`: Ensemble method - voting, stacking, weighted, or none (default: voting) / 集成方法 - 投票、堆叠、加权或无（默认：投票）
...
```
loanpractice/
├── data/                    # Data directory (place your CSV files here) / 数据目录（将您的 CSV 文件放在此处）
├── src/
│   ├── __init__.py
...
- **Gradient Boosting / 梯度提升**: Sequential boosting method / 顺序提升方法
...
### Deep Learning / 深度学习
- **Neural Network / 神经网络**: Multi-layer perceptron with dropout and batch normalization / 带有 dropout 和批归一化的多层感知器
...
预测脚本生成包含以下内容的 CSV 文件：

- `id`: Test sample identifier / 测试样本标识符
- `loan_status`: Predicted loan status (0 or 1) / 预测贷款状态（0 或 1）
